...
def convert_smol_llm_weights(llama, cfg: HookedTransformerConfig):
    state_dict = {}

    state_dict["embed.W_E"] = llama.model.text_model.embed_tokens.weight
    ...
    for l in range(cfg.n_layers):
        state_dict[f"blocks.{l}.ln1.w"] = llama.model.text_model.layers[l].input_layernorm.weight

        W_Q = llama.model.text_model.layers[l].self_attn.q_proj.weight
        W_K = llama.model.text_model.layers[l].self_attn.k_proj.weight
        W_V = llama.model.text_model.layers[l].self_attn.v_proj.weight
        ...
        W_O = llama.model.text_model.layers[l].self_attn.o_proj.weight
        ...
        state_dict[f"blocks.{l}.ln2.w"] = llama.model.text_model.layers[l].post_attention_layernorm.weight

        # in case of quantization,
        # parameters should stay as bitsandbytes.nn.modules.Params4bit
        if not cfg.load_in_4bit:
            state_dict[f"blocks.{l}.mlp.W_in"] = llama.model.text_model.layers[l].mlp.up_proj.weight.T
            state_dict[f"blocks.{l}.mlp.W_gate"] = llama.model.text_model.layers[l].mlp.gate_proj.weight.T
            state_dict[f"blocks.{l}.mlp.W_out"] = llama.model.text_model.layers[l].mlp.down_proj.weight.T
        else:
            state_dict[f"blocks.{l}.mlp.W_in"] = llama.model.text_model.layers[l].mlp.up_proj.weight
            state_dict[f"blocks.{l}.mlp.W_gate"] = llama.model.text_model.layers[l].mlp.gate_proj.weight
            state_dict[f"blocks.{l}.mlp.W_out"] = llama.model.text_model.layers[l].mlp.down_proj.weight
    ...
    state_dict["ln_final.w"] = llama.model.text_model.norm.weight
